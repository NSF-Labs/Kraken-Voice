...
import 'package:krak_en_voice/widgets/recording_state_chip.dart';
import 'package:krak_en_voice/screens/recording_detail_screen.dart';
...
class _FolderDetailScreenState extends State<FolderDetailScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _documentRepo = DocumentRepository(context.read<VaultService>());
    _loadDocuments();
    DocumentSummaryService.instance.states.addListener(_loadDocuments);
  ...
  @override
  void dispose() {
    DocumentSummaryService.instance.states.removeListener(_loadDocuments);
  ...
  Future<void> _loadDocuments() async {
    final documents = await _documentRepo.inFolder(widget.folder.id);
    if (mounted) setState(() => _documents = documents);
  }

  List<ImportedDocument> get _visibleDocuments {
    final now = DateTime.now();
    DateTime? start;
    if (_dateFilter == DateFilter.today) start = DateTime(now.year, now.month, now.day);
    if (_dateFilter == DateFilter.thisWeek) {
      final monday = now.subtract(Duration(days: now.weekday - 1));
      start = DateTime(monday.year, monday.month, monday.day);
    }
    if (_dateFilter == DateFilter.thisMonth) start = DateTime(now.year, now.month, 1);
    if (_dateFilter == DateFilter.custom) start = _customStart;
    final docs = _documents.where((d) => (start == null || !d.createdAt.isBefore(start)) &&
      (_dateFilter != DateFilter.custom || _customEnd == null || !d.createdAt.isAfter(_customEnd!))).toList();
    docs.sort((a, b) {
      final order = _sortField == SortField.title ? a.title.toLowerCase().compareTo(b.title.toLowerCase()) : a.createdAt.compareTo(b.createdAt);
      return _sortAscending ? order : -order;
    });
    return docs;
  }

  Widget _documentTile(ImportedDocument document) => Card(child: ListTile(
    leading: Icon(document.type == 'pdf' ? Icons.picture_as_pdf : Icons.description),
    title: Text(document.title),
    subtitle: Text('${document.type.toUpperCase()} document · ${document.summary == null ? "Ready to summarize" : "Summary saved"}'),
    onTap: _isSelectionMode ? null : () async {
      await Navigator.of(context).push(MaterialPageRoute(builder: (_) => DocumentDetailScreen(document: document)));
      await _loadDocuments();
    },
    trailing: _isSelectionMode ? null : IconButton(icon: const Icon(Icons.delete_outline), tooltip: 'Delete document', onPressed: () async {
      if (DocumentSummaryService.instance.states.value[document.id]?.working == true) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Wait for this document summary to finish before deleting it.')));
        return;
      }
      final confirm = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
        title: const Text('Delete document?'), content: const Text('This permanently removes the imported copy, extracted text and summary. Your original file is not changed.'),
        actions: [TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')),
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('Delete'))],
      ));
      if (confirm == true) {
        await _documentRepo.delete(document);
        await _loadDocuments();
      }
    }),
  ));
  ...
  Future<void> _deleteSelected() async {
    ...
    // Warn if speaker detection is actively running (high memory pressure)
    final engine = TranscriptionEngine();
    if (engine.isDiarizing.value) {
      final proceed = await showDialog<bool>(
        context: context,
        builder: (context) => AlertDialog(
          backgroundColor: KrakenColors.surfaceElevated,
          title: Text('Speaker detection in progress', style: KrakenText.displayMd()),
          content: Text(
            'Speaker detection is currently running on a recording. '
            'Deleting files now may cause instability. Wait until it finishes?',
          ...
          actions: [
            ...
            TextButton(
              ...
              child: Text('Delete Anyway', style: KrakenText.bodySm(color: Colors.redAccent)),
            ),
          ],
        ),
      );
    ...
    if (confirm == true) {
      try {
        await _folderRepo.trashRecordings(_selectedIds.toList());
        _exitSelectionMode();
        _loadRecordings();
        if (mounted) {
        ...
        debugPrint('[FolderDetail] Trash failed: $e');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Delete failed. Try again after speaker detection finishes.')),
          );
        }
        _exitSelectionMode();
      }
  ...
  // 2B-12: Batch move selected recordings to another folder
  Future<void> _moveSelected() async {
    ...
    final selectedFolder = await showDialog<Folder>(
      ...
      builder: (context) {
        return AlertDialog(
          backgroundColor: KrakenColors.surfaceElevated,
          title: Text('Move ${_selectedIds.length} recording${_selectedIds.length > 1 ? 's' : ''} to…', style: KrakenText.displayMd()),
          content: SizedBox(
    ...
    if (selectedFolder != null) {
      for (final id in _selectedIds) {
        await _folderRepo.moveRecording(id, selectedFolder.id);
      }
      final count = _selectedIds.length;
      _exitSelectionMode();
      _loadRecordings();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Moved $count recording${count > 1 ? 's' : ''} to ${selectedFolder.name}')),
        );
      }
    }
  }
  ...
  Future<void> _transcribeSelected() async {
    ...
    for (final rec in untranscribed) {
      await engine.queueJobWithDefaultLanguage(vault, rec.audioPath);
    }
  ...
  /// Import an audio file directly into this folder.
  /// Simplified version of the top-level import — no folder picker needed.
  Future<void> _importAudioToFolder() async {
    ...
    if (isDocumentFile(selectedFile.name)) {
      if (!mounted) return;
      await importDocumentToFolder(context, selectedFile, widget.folder.id);
    ...
    // Create recording in this folder
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_isSelectionMode) ...[
            ...
            // 2B-12: Move selected to another folder
            IconButton(
              icon: const Icon(Icons.drive_file_move_outline, color: KrakenColors.accent),
            ...
            if (!widget.folder.isDefault)
              IconButton(
                icon: const Icon(Icons.delete_outline, color: Colors.redAccent),
                onPressed: () async {
                  final recordingsCount = (await _folderRepo.getRecordingsForFolder(widget.folder.id)).length + (await _documentRepo.inFolder(widget.folder.id)).length;
                  ...
                  if (recordingsCount == 0) {
                    ...
                    final result = await showDialog<String>(
                      ...
                      builder: (context) => AlertDialog(
                        ...
                        title: Text('Delete folder?', style: KrakenText.displayMd(color: Colors.redAccent)),
                        content: Text(
                          'This folder contains $recordingsCount file${recordingsCount == 1 ? '' : 's'}. What would you like to do with them?',
                          style: KrakenText.bodyMd(),
                        ),
                        actions: [
                    ...
                    if (result == 'delete_all') {
                      ...
                      final target = await showDialog<Folder>(
                        ...
                        builder: (context) => AlertDialog(
                          backgroundColor: KrakenColors.surfaceElevated,
                          title: Text('Move files to…', style: KrakenText.displayMd()),
                          content: SizedBox(
                            width: double.maxFinite,
                            child: ListView.builder(
                              shrinkWrap: true,
      ...
      body: _isLoading
          ...
          : Column(
              children: [
                ...
                // Recording list
                Expanded(
                  child: _recordings.isEmpty && _visibleDocuments.isEmpty
                    ? Center(
                        child: Padding(
                            ...
                            children: [
                              ...
                              Text(
                                _dateFilter != DateFilter.all
                                    ...
                                    : 'Recordings will appear here after you\nrecord or import audio, PDFs or Word documents.',
                  ...
                  builder: (context, progressMap, _) {
                    return ListView.builder(
                      ...
                      itemCount: _recordings.length + _visibleDocuments.length,
                      itemBuilder: (context, index) {
                        final documents = _visibleDocuments;
                        if (index < documents.length) return _documentTile(documents[index]);
                        final rec = _recordings[index - documents.length];
