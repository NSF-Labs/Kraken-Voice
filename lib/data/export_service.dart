...
class KrakenExportService {
  ...
  /// Injects speaker labels into transcript text at turn boundaries.
  ///
  /// Two paths:
  ///   • If [transcriptSegments] is provided (Whisper's per-segment
  ///     timestamps), each segment is aligned to the diarization speaker
  ///     that owns its midpoint. Accurate to the segment.
  ///   • Otherwise the legacy proportional char-position mapping is used,
  ///     for back-compat with recordings transcribed before the segments
  ///     column was added.
  static String injectSpeakerLabels(
    String text,
    ...
    if (segments.isEmpty || text.isEmpty) return text;
    ...
    void flushChunk() {
      final speaker = lastSpeaker;
      if (speaker == null || buf.isEmpty) return;
      final label = labels[speaker] ?? 'Speaker ${speaker + 1}';
      final ts = _formatTimestamp(chunkStartTs);
      if (result.isNotEmpty) result.writeln();
      result.writeln('$label $ts:');
      result.writeln(buf.toString().trim());
      buf.clear();
    }

    if (transcriptSegments != null && transcriptSegments.isNotEmpty) {
      // Split at sentence boundaries first so multi-sentence Whisper
      // segments don't collapse to a single speaker assignment.
      final fineGrained = splitWhisperSegmentsBySentence(transcriptSegments);

      // ── Accurate path: align Whisper segments to diarization speakers ──
      for (final seg in fineGrained) {
        final cleaned = seg.text.trim();
        if (cleaned.isEmpty) continue;
        // Overlap-weighted majority instead of midpoint lookup — see
        // DiarizationSegment.speakerByOverlap.
        final speaker = DiarizationSegment.speakerByOverlap(
              seg.startSeconds,
              seg.endSeconds,
              merged,
            ) ??
    ...
    // ── Legacy proportional path (no Whisper segments stored) ──
    double totalDurationS = totalDurationMs / 1000.0;
    if (totalDurationS <= 0) {
      totalDurationS =
          merged.map((s) => s.endSeconds).reduce((a, b) => a > b ? a : b);
    }
    if (totalDurationS <= 0) return text;

    final totalLen = text.length;
    final wordPattern = RegExp(r'\S+');
    final wordMatches = wordPattern.allMatches(text).toList();
    if (wordMatches.isEmpty) return text;

    for (final m in wordMatches) {
      final charMid = (m.start + m.end) / 2;
      final fraction = (charMid / totalLen).clamp(0.0, 1.0);
  ...
  /// Formats seconds into a readable timestamp string: [M:SS] or [H:MM:SS].
  static String _formatTimestamp(double totalSeconds) {
  ...
  pw.Widget _buildHeader(BrandConfig brand) {
    return pw.Container(
      ...
      decoration: pw.BoxDecoration(
        border: pw.Border(
          bottom: pw.BorderSide(color: brand.accentColor, width: 0.5),
        ),
      ),
      child: pw.Text(
        brand.headerText ?? '',
  ...
  pw.Widget _buildFooter(pw.Context context, BrandConfig brand) {
    final footerText = brand.hasFooter
        ? brand.footerText!
  ...
  List<pw.Widget> _buildListItems(
    String heading,
    dynamic items,
    BrandConfig brand,
  ) {
  ...
  List<pw.Widget> _buildChecklistItems(
    String heading,
    dynamic items,
    BrandConfig brand,
  ) {
  ...
  Future<File> exportDocx({
    ...
    body.writeln(
      _docxParagraph('$dateStr  •  $durationStr', style: 'Subtitle'),
    );
    if (brand.hasHeader) {
      body.writeln(
        _docxParagraph(
          brand.headerText!,
          italic: true,
          color: brand.accentColorHex.replaceAll('#', ''),
        ),
      );
    }
    ...
    // Footer
    body.writeln(_docxParagraph(''));
    final docxFooter = brand.hasFooter
        ? brand.footerText!
        : 'Generated by The Kraken';
...
${body.toString()}
  ...
  String _docxParagraph(
    ...
    final rPr = runProps.isNotEmpty
        ? '<w:rPr>${runProps.toString()}</w:rPr>'
        : '';
